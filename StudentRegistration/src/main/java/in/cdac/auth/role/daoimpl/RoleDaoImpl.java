...
public class RoleDaoImpl implements RoleDao {
	...
	public ResultDataMap getRoleByRoleId(Integer roleId) {
		...
		/*List<Authorities> assignedAuthorities = (List<Authorities>) session.creatennNativeQuery(
				"select * from authorities where id  in (select authorityId from role_authorities where roleId=:roleId)")
				.setParameter("roleId", roleId).addEntity(Authorities.class).list();*/
		...
		/*@SuppressWarnings("unchecked")
		List<Authorities> notAssignedAuthorities = (List<Authorities>) session.creatennNativeQuery(
				"select * from authorities where id not in (select authorityId from role_authorities where roleId=:roleId)")
				.setParameter("roleId", roleId).addEntity(Authorities.class).list();
		*/
	...
	public ResultDataMap getAllRoleAuthorities() {
		...
		for (MstRole role : roleList) {
			/*@SuppressWarnings("unchecked")
			List<Authorities> assignedAuthoritiesList = (List<Authorities>) session.creatennNativeQuery(
					"select * from Authorities where id  in (select authorityId from role_authorities where roleId=:roleId)")
					.setParameter("roleId", role.getRoleId()).addEntity(Authorities.class).list();*/
