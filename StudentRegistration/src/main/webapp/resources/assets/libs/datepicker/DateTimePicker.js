...
$.DateTimePicker = $.DateTimePicker || {
	...
	defaults:  //Plugin Defaults
	{
		...
		titleContentDate: "Set Dose Date",
	...
	DateTimePicker.prototype = {
		...
		_addEventHandlersForPicker: function()
		{
			...
			$(oDTP.element).find('.dtpicker-buttonSet').click(function(e)
			{
				...
				var dt = $("#date_of_discharge").val();
				if ( dt != "") {
					$("#date_of_discharge").parent().parent().parent().removeClass("floating-label");
				}

				var dt = $("#neuritisDate").val();
				if ( dt != "") {
					$("#neuritisDate").parent().parent().parent().removeClass("floating-label");
				}

				var dt = $("#predinsolone_date").val();
				if ( dt != "") {
					$("#predinsolone_date").parent().parent().parent().removeClass("floating-label");
				}

				var dt = $("#mcr_footware_date").val();
				if ( dt != "") {
					$("#mcr_footware_date").parent().parent().parent().removeClass("floating-label");
				}

				var dt = $("#referral_rcs_date").val();
				if ( dt != "") {
					$("#referral_rcs_date").parent().parent().parent().removeClass("floating-label");
				}
			});
			...
			$(oDTP.element).find('.dtpicker-buttonClear').click(function(e)
			{
				...
				var dt = $("#date_of_discharge").val();
				if ( dt == "") {
					if (!$("#date_of_discharge").parent().parent().parent().hasClass("floating-label")) {
						$("#date_of_discharge").parent().parent().parent().addClass("floating-label");
					}
				}

				var dt = $("#neuritisDate").val();
				if ( dt == "") {
					if (!$("#neuritisDate").parent().parent().parent().hasClass("floating-label")) {
						$("#neuritisDate").parent().parent().parent().addClass("floating-label");
					}
				}

				var dt = $("#predinsolone_date").val();
				if ( dt == "") {
					if (!$("#predinsolone_date").parent().parent().parent().hasClass("floating-label")) {
						$("#predinsolone_date").parent().parent().parent().addClass("floating-label");
					}
				}

				var dt = $("#mcr_footware_date").val();
				if ( dt == "") {
					if (!$("#mcr_footware_date").parent().parent().parent().hasClass("floating-label")) {
						$("#mcr_footware_date").parent().parent().parent().addClass("floating-label");
					}
				}

				var dt = $("#referral_rcs_date").val();
				if ( dt == "") {
					if (!$("#referral_rcs_date").parent().parent().parent().hasClass("floating-label")) {
						$("#referral_rcs_date").parent().parent().parent().addClass("floating-label");
					}
				}
