...
public class ImsSecurityConfig extends WebSecurityConfigurerAdapter{
	...
	/*	//use jdbc authentication
		// auth.jdbcAuthentication().dataSource(securityDataSource);
		 auth.jdbcAuthentication().dataSource(securityDataSource)
		   ...
		   //  "select userId username,usercol role from User where userEmail=?");
		   //"select username,authority from Authorities where username=?");
		 "SELECT u.userId username,am.authority authority FROM UserRole u,authoritiesMaster am,role_authorities ra"
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
			...
			.antMatchers("/site/admin/**").hasAnyRole("SITE_SETUP","SUPER_ADMIN")
			.antMatchers("/site/content/**").hasAnyRole("SITE_CONTENT_MANAGMENT","SUPER_ADMIN")
			.antMatchers("/site/**").hasAnyRole("SITE_CREATION_MANAGMENT","SUPER_ADMIN")
			...
			.antMatchers("/hr/**").hasAnyRole("HR","SUPER_ADMIN","SUPER_ADMIN")
			.antMatchers("/programme/**").hasAnyRole("PROGRAMME_ADMINSTRAION","SUPER_ADMIN")
			.antMatchers("/file/admin/**").hasAnyRole("FTS_ADMIN","SUPER_ADMIN")
			.antMatchers("/lib/admin/**").hasAnyRole("LIB_ADMIN","SUPER_ADMIN")
			.antMatchers("/inventory/manage/**").hasAnyRole("INVENTORY_MANAGMENT","SUPER_ADMIN")
			.antMatchers("/hostel/admin**").hasAnyRole("HOSTEL_ADMINSTRATION","SUPER_ADMIN")
			.antMatchers("/auth/createUser").hasAnyRole("CREATE_USER")
			.antMatchers("/auth/viewUser").hasAnyRole("VIEW_USER")
			
			.antMatchers("/auth/**").hasAnyRole("AUTHENTICATION_MANAGMENT","SUPER_ADMIN")
