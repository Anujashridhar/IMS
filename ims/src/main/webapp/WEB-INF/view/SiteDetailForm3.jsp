						...
						<form:form class="col s12" modelAttribute="site"
							...
							<div class="row" id="row1">
								...
								<div class="input-field col s4">
									...
									<label for="icon_prefix" class="active">No of boxes of contents as shown in demo Template</label>
								...
								<div id="loc_div" class="input-field col s3">
									<c:choose>
									<c:when test="${site.siteData.noOfHierarchyLevels ==null || site.siteData.noOfHierarchyLevels ==0  }">
											<div class="input-field " id="locNameDiv1" style="display: none;">
									...
									<div class="row" id="row${cnt.index }">
										<div class="input-field col s3">
											...
											<form:input value="${menu.key }"
												...
												id="menu_${menu_count }" class="validate active" />
										...
										<c:if test="${empty menu.value.sumenuList }">
											<div class="col s2" id="check_div_${cnt.index }">
												<input type="checkbox" id="check_${cnt.index}" /> <label
													for="test6">Sub menu</label>
											</div>
										...
										<c:if test="${not empty menu.value.sumenuList }">
											<div class="col s2" id="check_div_${cnt.index }">
												<input type="checkbox" id="check_${cnt.index}"
													checked="checked" /> <label for="test6">Sub menu</label>
											</div>
<button id="b1" class="btn add-more" type="button" >+</button>
											...
											<c:forEach items="${menu.value.sumenuList }" var="submenu"
												...
												<c:set var="menu_count" value="${menu_count+1 }"></c:set>
													...
													<form:input value="${submenu.menuName }"
														path="menuDetails[${menu_count }].menuName"
														id="field${menu_count }" class="validate active" />
													<form:hidden value="${submenu.url }"
														path="menuDetails[${menu_count }].menuUrl"
														id="fieldu${menu_count }" />
													<form:hidden value="${submenu.parentId }"
														path="menuDetails[${menu_count }].parentId"
														id="fieldp${menu_count }" class="validate active" />
													<form:hidden value="${submenu.menuId }"
														path="menuDetails[${menu_count }].menuId"
														id="fielda${menu_count }" class="validate active" />
								...
								<div class="row" id="row${menu_count }">
									...
									<div class="col s2" id="check_div_${menu_count}">
										<input type="checkbox" id="check_${menu_count}" /> <label
											for="test6">Sub menu</label>
									</div>
									<div class="input-field col s3" id="sub_cnt_${menu_count }">
										<input id="submenu_1" type="tel" class="validate"
											onchange="addinputFields(this.id)"><br /> <label
											for="icon_telephone">No.of sub menu</label>
									</div>
									<c:set var="menu_count" value="${menu_count+1}"></c:set>
									<div id="container_1" class="input-field col s3">
										<form:input path="menuDetails[${menu_count }].menuName"
											id="menu_${cnt.index }" class="validate active" />
										<form:hidden path="menuDetails[${menu_count }].parentId"
											value="-1" id="menu_${menu_count }" class="validate active" />
